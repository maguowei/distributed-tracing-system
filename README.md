...
但是随着大型单体应用拆分为微服务，服务之间的依赖和调用变得极为复杂，这些服务可能是不同团队开发的，可能基于不同的语言，微服务之间可能是利用RPC, RESTful API, 也可能是通过消息队列实现调用或通讯。如何理清服务依赖调用关系，如何在这样的环境下迅速debug和排查线上问题, 追踪各服务处理耗时，查找服务性能瓶颈, 合理对服务的容量评估都变成一个棘手的事情。
...
Logging 主要记录一些离散的事件，应用往往通过将定义好格式的日志信息输出到文件，然后用日志收集程序收集起来用于分析和聚合。目前已经有 `ELK` 这样的成熟方案, 虽然日志可以用时间将所有点事件串联起来，但是却不能
 
Metric 往往是一些聚合的信息，相比Logging 丧失了一些具体信息，但是占用的空间会更小，可以用于监控和报警，在这方面，方面 `prometheus` 已经基本上成为了事实上的标准。

Tracing 它在单次请求的范围内，处理信息。 任何的数据、元数据信息都被绑定到系统中的单个事务上。分布式追踪系统可以帮助我们更好的理解系统的行为、辅助调试和排查性能问题
